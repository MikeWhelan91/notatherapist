...
const createConversation = ({ weeklyReview, monthlyReview, profile, cadence = "weekly" }) => {
  const review = cadence === "monthly" ? monthlyReview : weeklyReview;
  const maxTurns = cadence === "monthly" ? MONTHLY_MAX_TURNS : MAX_TURNS;
  ...
  const patternText = patterns.length
    ...
    : "a few early themes";
  const concreteHook = review?.primaryLoop || review?.patternShift || review?.suggestion || review?.summary || patternText;
  ...
  const contextHints = [
    ...
    ...(Array.isArray(review?.patterns) ? review.patterns.slice(0, cadence === "monthly" ? 4 : 3) : []),
    review?.suggestion,
    review?.patternShift,
  ...
  return {
    ...
    title: cadence === "monthly" ? "Monthly review" : "Weekly check-in",
    date: now,
    preview: opener,
    messages: [
      {
...
const createAIConversation = async ({ weeklyReview, monthlyReview, profile, cadence = "weekly" }) => {
  const base = createConversation({ weeklyReview, monthlyReview, profile, cadence });
  ...
  return {
    ...
    conversation: {
      ...base,
      title: generated.title || (cadence === "monthly" ? "Monthly review" : "Weekly check-in"),
...
const replyToConversation = ({ text, action, remainingTurns, profile, conversation, planTier = "free" }) => {
  ...
  const cadence = conversation?.reviewCadence === "monthly" ? "monthly" : "weekly";
  const baseMaxTurns = cadence === "monthly" ? MONTHLY_MAX_TURNS : MAX_TURNS;
  const deeperMaxTurns = baseMaxTurns + DEEPER_BONUS_TURNS;
  ...
  if (action === "Go deeper") {
    ...
    return {
      ...
      reply: cadence === "monthly"
        ? "We can go deeper. Which thread from the last 4 weeks feels most important to change next month?"
        : "We can go deeper. What part feels most emotionally charged or most repeated this week?",
      replyContext: cadence === "monthly"
        ? "This deeper pass builds on the monthly review and prior replies in this check-in."
        : "This deeper pass builds on your weekly patterns and prior replies in this check-in.",
      suggestedGoal: null,
      maxTurns: deeperMaxTurns,
      deepeningUsed: true,
      phase: "deeper"
  ...
  if (!turnsLeft || action === "End for today") {
    return {
      ...
      replyContext: "",
      suggestedGoal: null,
      maxTurns: deepeningUsed ? deeperMaxTurns : baseMaxTurns,
      deepeningUsed,
  ...
  const suggestedGoal = action === "Give me one action"
    ? {
        ...
        reason: cadence === "monthly" ? "Agreed during the monthly review." : "Agreed during the weekly check-in.",
        createdAt: new Date().toISOString(),
        dueDate: new Date(Date.now() + (cadence === "monthly" ? 30 : 3) * 24 * 60 * 60 * 1000).toISOString(),
        status: "active",
...
const replyToAIConversation = async ({ text, action, remainingTurns, profile, conversation, planTier = "free" }) => {
  ...
  if (safety.level === "crisis") {
    return {
      ...
      suggestedGoal: null,
      actions: [],
      maxTurns: conversation?.maxTurns || (conversation?.reviewCadence === "monthly" ? MONTHLY_MAX_TURNS : MAX_TURNS),
  ...
  const generated = await safeGenerateStructured({
    ...
    input: {
      ...
      cadence: conversation?.reviewCadence === "monthly" ? "monthly" : "weekly",
      phase: base.phase
    },
    maxOutputTokens: 320
  });
  ...
  const suggestedGoal = action === "Give me one action" && generated.suggestedGoalTitle
    ? {
        ...
        reason: generated.suggestedGoalReason || (conversation?.reviewCadence === "monthly" ? "Agreed during the monthly review." : "Agreed during the weekly check-in."),
        createdAt: new Date().toISOString(),
        dueDate: new Date(Date.now() + (conversation?.reviewCadence === "monthly" ? 30 : 7) * 24 * 60 * 60 * 1000).toISOString(),
        status: "active",
        sourceConversationID: null,
...
const guidedReply = ({ text, action, remainingTurns, conversation }) => {
  ...
  if (prompt.includes("action")) {
    const cadence = conversation?.reviewCadence === "monthly" ? "monthly" : "weekly";
    const base = firstHint ? trimSnippet(firstHint, 80) : `the strongest ${cadence} pattern`;
    return cadence === "monthly"
      ? `One useful 30-day step: choose one small action linked to ${base}, repeat it weekly, and review whether the pattern softens.`
      : `One useful 7-day step: choose one small action linked to ${base}, do it before tomorrow evening, then stop.`;
  }
...
const buildReplyContext = ({ conversation, action, text }) => {
  ...
  if (action === "Give me one action") {
    const cadence = conversation?.reviewCadence === "monthly" ? "monthly" : "weekly";
    if (hints[0]) {
      return `Action based on repeated ${cadence} pattern: ${hints[0]}`;
    }
  ...
  if (hints[0]) {
    const cadence = conversation?.reviewCadence === "monthly" ? "monthly" : "weekly";
    return `This reply references your ${cadence} pattern: ${hints[0]}`;
  }
