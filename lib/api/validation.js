...
const normalizeDate = (value, defaultDate = new Date()) => {
...
const normalizeProfile = (profile = {}) => ({
  ...
  focusAreas: cleanStringArray(profile.focusAreas),
...
const normalizeGoals = (goals) => {
  ...
  return goals.slice(0, 8).map((goal) => ({
    ...
    feedback: cleanString(goal.feedback).slice(0, 32)
  })).filter((goal) => goal.title);
};

const normalizeCalmSessions = (sessions) => {
  if (!Array.isArray(sessions)) {
    return [];
  }

  return sessions.slice(0, 24).map((session) => ({
    id: cleanString(session.id) || crypto.randomUUID(),
    pathway: cleanString(session.pathway).slice(0, 40),
    breathingMode: cleanString(session.breathingMode).slice(0, 60),
    startedAt: normalizeDate(session.startedAt),
    endedAt: normalizeDate(session.endedAt),
    duration: Math.max(0, Number(session.duration) || 0),
    startingMood: MOODS.has(session.startingMood) ? session.startingMood : "okay",
    targetMood: MOODS.has(session.targetMood) ? session.targetMood : "good",
    helpfulness: cleanString(session.helpfulness).slice(0, 24)
  })).filter((session) => session.pathway);
};
