...
const createWeeklyReview = ({ entries, profile = {}, healthSummary, goals = [], planTier = "free" }) => {
  if (!entries.length) {
    return {
      canReview: false,
...
const createAIWeeklyReview = async ({ entries, profile, healthSummary, goals = [], planTier = "free" }) => {
  ...
  await moderateEntries(entries);
  const eligibility = createWeeklyReview({ entries, profile, healthSummary, goals, planTier });
...
const createMonthlyReview = ({ entries, weeklyReviews = [], healthSummary, goals = [], planTier = "free", profile = {} }) => {
  if (planTier !== "premium") {
...
const createAIMonthlyReview = async ({ entries, weeklyReviews = [], profile, healthSummary, goals = [], planTier = "free" }) => {
  const eligibility = createMonthlyReview({ entries, weeklyReviews, healthSummary, goals, planTier, profile });
  if (!eligibility.canReview) {
    return { ...eligibility, source: "local" };
  }
  await moderateEntries(entries);
