...
const dailyEmotionalRead = ({ sortedEntries, lowerText, averageMood, topTheme, topIssue }) => {
  ...
  if (topIssue) {
    return `${topIssue.label} appears as a key issue in today's entries.`;
  }
...
const dailyPattern = ({ sortedEntries, lowerText, averageMood, topTheme, topIssue, healthSummary }) => {
  ...
  if (topIssue) {
    ...
    return `${topIssue.label} was mentioned in ${countText}.`;
  }
...
const dailyAction = ({ sortedEntries, lowerText, themes }) => {
  if (lowerText.includes("drove") || lowerText.includes("driving")) {
    return "If the drive feels tense again, name one thing that helped today before leaving.";
  }
  if (sortedEntries.some((entry) => entry.entryType === "win")) {
...
const suggestedGoal = ({ sortedEntries, lowerText, themes }) => {
  ...
  if (lowerText.includes("drove") || lowerText.includes("driving")) {
    return {
      title: "Capture one driving anchor",
      reason: "You referenced driving directly, so this is testable tomorrow."
    };
  }
  if (sortedEntries.some((entry) => entry.entryType === "win")) {
    return {
      title: "Repeat one useful condition",
      reason: "Today's win gives one concrete condition you can reuse."
    };
  }
  if (themes.includes("Work")) {
    return {
      title: "Close one work loop",
      reason: "Work appeared repeatedly, so one closure is measurable."
    };
  }
  if (themes.includes("Anxiety")) {
    return {
      title: "Use a 60-second reset once",
      reason: "Anxiety appeared today, so this is a direct one-day experiment."
    };
  }
  return {
    title: "",
...
const issueMatchers = [
  { key: "anxiety", label: "Anxiety", includes: ["anxiety", "anxious"], excludes: ["no anxiety", "not anxious", "without anxiety"] },
