...
const createAIDailyReview = async ({ date, entries, profile, healthSummary }) => {
...
const createWeeklyReview = ({ entries, healthSummary }) => {
  if (!entries.length) {
    return {
      canReview: false,
...
const createAIWeeklyReview = async ({ entries, profile, healthSummary }) => {
  const fallback = createWeeklyReview({ entries, healthSummary });
...
const dailyEmotionalRead = ({ sortedEntries, lowerText, averageMood }) => {
  if (sortedEntries.some((entry) => entry.entryType === "win") || averageMood >= 4) {
    return "Today reads steadier than usual.";
  }
...
const suggestedGoal = ({ sortedEntries, lowerText, themes }) => {
  if (lowerText.includes("drove") || lowerText.includes("driving")) {
    return {
      ...
      reason: "Your review pointed to a specific useful moment."
    };
  }
  if (sortedEntries.some((entry) => entry.entryType === "win")) {
    return {
      title: "Record one useful condition",
      reason: "Wins are easier to repeat when the conditions are clear."
    };
  }
  if (themes.includes("Work")) {
    return {
