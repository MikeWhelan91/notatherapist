...
const createMonthlyReview = ({ entries, weeklyReviews = [], healthSummary, goals = [], planTier = "free" }) => {
  if (planTier !== "premium") {
  ...
  return {
    ...
    monthlyReview: {
      ...
      progress: completedGoals ? `You completed ${completedGoals} monthly next step${completedGoals === 1 ? "" : "s"}.` : "No completed monthly next steps recorded yet.",
      ...
      summary: `${distinctDays} active days and ${sortedEntries.length} entries across ${dateRangeText}.`,
      moodRange,
      patterns,
      risk: patterns.length ? "Keep the monthly goal tied to the clearest repeated signal, not every issue at once." : "",
      ...
      goalFollowThrough: completedGoals ? `Goal follow-through is visible: ${completedGoals} completed.` : "Goal follow-through needs explicit completed next steps.",
      progressSignal: weeklyReviews.map((review) => review.progressSignal).filter(Boolean).slice(-1)[0] || "",
...
const createAIMonthlyReview = async ({ entries, weeklyReviews = [], profile, healthSummary, goals = [], planTier = "free" }) => {
  const eligibility = createMonthlyReview({ entries, weeklyReviews, healthSummary, goals, planTier });
  if (!eligibility.canReview) {
    return { ...eligibility, source: "local" };
  }
  await moderateEntries(entries);
  const generated = await safeGenerateStructured({
    ...
    input: { profile, healthSummary, planTier, goals, weeklyReviews, entries: entries.map(publicEntry) },
    maxOutputTokens: 900
  });
  if (!generated) {
